...
class QuizManager {
    ...
    private var _totalAnswers: UInt8 = 0
    private var _totalCorrectAnswers: UInt8 = 0
    ...
    var totalAnswers: UInt8 {
        return self._totalAnswers
    }
    
    var totalCorrectAnswers: UInt8 {
        return self._totalCorrectAnswers
    }
    
    func refreshQuiz() {
        let randon = Int.random(in: 0...Int(quizes.count - 1))
        let quizData = quizes[randon]
